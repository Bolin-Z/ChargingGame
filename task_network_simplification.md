...
##### 5.2.10.6 实施完成记录 ✅ (2025-12)

**已完成任务**:
- [x] 在 `__route_choice_update` 中实现方案 C（Gap敏感 + 时间衰减）
- [x] 添加迭代轮数参数传递机制
- [x] 参数扫描实验（γ × α 组合）
- [x] 在三个网络上验证改进效果
- [x] 确定最终的 γ 和 α 参数值
...
扫描范围：γ ∈ [5, 10, 15, 20], α ∈ [0.02, 0.03, 0.05, 0.08]

**最佳参数配置**:

| 网络 | γ | α | demand_multiplier | 收敛阈值 |
|-----|---|---|-------------------|---------|
| **Sioux Falls** | 5 | 0.08 | 1.0 | 1% |
| **Berlin** | 10 | 0.05 | 2.5 | 3% |
| **Anaheim** | 15 | 0.08 | 0.5 | 3% |

**参数选择要点**:
- 较小网络（SF）需要较低的 γ，避免早期过度切换
- 较大 α（0.08）普遍表现较好，快速衰减避免后期振荡
- Berlin 的 demand_multiplier 从 2.0 提升到 2.5，增加博弈难度

##### 5.2.10.7 验证测试结果 (2025-12)

**测试工具**: `verify_ue_parameters.py`
**测试方法**: 强制运行 100 轮迭代，记录每轮数据并绘制收敛曲线

**收敛验证结果**:

| 网络 | 首次收敛轮数 | 收敛时 GM | 100轮 GM | 变化 | 完成率 |
|-----|------------|----------|----------|------|-------|
| **Sioux Falls** | 22轮 | 0.87% | 0.90% | +0.03% | 100% |
| **Berlin** | 25轮 | 2.75% | 3.75% | +1.00% | 100% |
| **Anaheim** | 82轮 | 2.87% | 3.52% | +0.65% | 96.6% |

**关键发现**:

1. **收敛检测机制验证有效**：所有网络在收敛点后继续迭代，GM 反而上升，说明应在收敛后立即停止

2. **振荡现象确认**：
   - Berlin 振荡最明显：从 2.75% 上升到 3.75%（+36% 相对增幅）
   - 这是 Gap-based 切换机制在均衡点附近的正常行为
   - 收敛检测 + 提前停止是正确的策略

3. **completed_ratio 问题解决**：
   - Sioux Falls: 100% ✅
   - Berlin: 100% ✅（之前参数配置下仅 37%~53%）
   - Anaheim: 96.6% ✅

**收敛曲线图表**: `results/verification/convergence_curves.png`

##### 5.2.10.8 配置文件更新

已更新各网络的 `settings.json`:
...
// siouxfalls_settings.json
{
    ...
    "ue_convergence_threshold": 0.01
}

// berlin_friedrichshain_settings.json
{
...
|-------|-------|-------|-------|---------|-----------|-------------|
| **Sioux Falls** | 24 | 76 | 528 | 4 | 良好 | GM<1% |
| **Berlin** | 191 | 451 | 506 | 20 | 中等 | GM<3% |
| **Anaheim** | 416 | 914 | 1406 | 40 | 中等 | GM<3% |

### 6.2 最终参数配置 (2025-12)

| 数据集 | γ | α | demand_multiplier | 收敛轮数 | 收敛时GM | 完成率 |
|-------|---|---|-------------------|---------|---------|-------|
| **Sioux Falls** | 5 | 0.08 | 1.0 | 22轮 | 0.87% | 100% |
| **Berlin** | 10 | 0.05 | 2.5 | 25轮 | 2.75% | 100% |
| **Anaheim** | 15 | 0.08 | 0.5 | 82轮 | 2.87% | 96.6% |
...
2. **α（衰减速率）**：0.05~0.08 普遍适用
   - 较大 α 有助于后期稳定
   - Berlin 使用 0.05 因其网络特性需要更多迭代

3. **demand_multiplier**：需根据网络拥堵特性调整
   - SF：1.0（理论网络，无需放大）
   - Berlin：2.5（真实网络，需要适度拥堵）
   - Anaheim：0.5（原始需求已较大）
