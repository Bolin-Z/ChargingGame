# UE-DTA 参数优化任务记录

## 1. 任务背景

基于 `task_network_simplification.md` 文档中 5.2.10 节提出的**方案C（Gap敏感 + 时间衰减）**切换机制，已在 `EVCSChargingGameEnv.py` 中实现。

切换概率公式：
$$P_{switch}(n) = \min(1, \gamma \cdot gap_{rel}) \cdot \frac{1}{1 + \alpha \cdot n}$$

- $\gamma$：Gap敏感度系数
- $\alpha$：时间衰减速率
- $n$：当前迭代轮数
- $gap_{rel}$：相对成本差

## 2. 参数扫描实验

### 2.1 实验配置

- **扫描脚本**：`sweep_ue_parameters.py`
- **执行时间**：2025-12-18
- **参数范围**：
  - gamma: [5, 10, 15, 20]
  - alpha: [0.02, 0.03, 0.05, 0.08]
- **测试网络**：siouxfalls, berlin_friedrichshain, anaheim
- **总组合数**：16 × 3 = 48

### 2.2 实验结果（原始，有缺陷）

| 网络 | 最佳 γ | 最佳 α | GM | P95 | 迭代次数 | Completed | 状态 |
|-----|--------|--------|-----|-----|---------|-----------|------|
| **Sioux Falls** | 15 | 0.03 | 0.59% | 3.04% | 10 | **53.9%** | ⚠️ 虚假收敛 |
| **Berlin** | 10 | 0.05 | 1.60% | 10.26% | 4 | 100% | ✅ 良好 |
| **Anaheim** | 15 | 0.08 | 2.69% | 16.55% | 53 | 96.5% | ✅ 良好 |

### 2.3 问题分析与修复

#### 2.3.1 问题根源

原评分函数只考虑 GM 和迭代次数，**未考虑 completed_ratio**：
```python
score = 100 * (1 - 0.7 * gm_norm - 0.3 * iter_norm)
```

导致 Sioux Falls 的 γ=15, α=0.03 配置被错误选为"最佳"：
- GM=0.59%（看似很低）+ iterations=10（很少）→ 高分
- 但 completed_ratio=53.9%，实际是**虚假收敛**

#### 2.3.2 虚假收敛机制

1. 高 γ + 低 α → 早期迭代切换概率接近 100%（对比固定概率 5%）
2. 大量车辆切换到"最优"路径 → 该路径严重拥堵
3. 车辆无法在 simulation_time 内完成 → completed_ratio 下降
4. 未完成车辆不参与 gap 计算 → 少量完成车辆的 gap 很小
5. 快速"收敛"（实为假收敛）

#### 2.3.3 修复方案

添加硬约束：**completed_ratio < 95% 的配置直接排除**

分析脚本：`analyze_sweep_results.py`

### 2.4 修正后的最佳参数

| 网络 | 最佳 γ | 最佳 α | GM | P95 | 迭代次数 | Completed | 状态 |
|-----|--------|--------|-----|-----|---------|-----------|------|
| **Sioux Falls** | 5 | 0.08 | 0.79% | 4.63% | 24 | 100% | ✅ 修正 |
| **Berlin** | 10 | 0.05 | 1.60% | 10.26% | 4 | 100% | ✅ 不变 |
| **Anaheim** | 15 | 0.08 | 2.69% | 16.55% | 53 | 96.5% | ✅ 不变 |

### 2.5 关键发现

1. **Berlin 表现优异**
   - 100% completed_ratio
   - 仅需 4 轮迭代即收敛
   - GM < 2%，满足收敛标准

2. **Sioux Falls 问题已修复**
   - 原 γ=15, α=0.03 为虚假收敛
   - 修正为 γ=5, α=0.08，completed_ratio=100%
   - 低 γ 值避免早期过度切换

3. **Anaheim 基本正常**
   - 96.5% completed_ratio
   - 迭代次数较多（53轮），但最终收敛

4. **参数规律**
   - 较小网络（Sioux Falls）需要较低的 γ
   - 较大 α（0.08）普遍表现较好（快速衰减避免过度切换）

## 3. 已解决问题

### 3.1 ✅ Sioux Falls completed_ratio 异常

**问题描述**：Sioux Falls 作为 24 节点的理论网络，completed_ratio 应接近 100%，但实测仅 53.9%。

**根因分析**：
- ~~`simulation_time` 设置过短~~ ❌ 不是根因
- ~~`demand_multiplier` 过高导致网络拥堵~~ ❌ 不是根因
- ✅ **评分函数缺陷**：未考虑 completed_ratio，导致选出"虚假收敛"参数
- ✅ **虚假收敛**：高 γ + 低 α 导致早期过度切换，车辆无法按时完成

**解决方案**：
- 添加 completed_ratio ≥ 95% 硬约束
- Sioux Falls 参数从 γ=15, α=0.03 修正为 γ=5, α=0.08

### 3.2 ✅ 更新各网络 settings.json

已更新配置文件：

| 网络 | ue_switch_gamma | ue_switch_alpha | 状态 |
|-----|-----------------|-----------------|------|
| Sioux Falls | 5 | 0.08 | ✅ 已更新 |
| Berlin | 10 | 0.05 | ✅ 已确认 |
| Anaheim | 15 | 0.08 | ✅ 已更新 |

### 3.3 ✅ Berlin demand_multiplier 优化

**问题发现**：参数扫描中 Berlin 网络所有 16 个 γ/α 组合都在 **4 轮**迭代内收敛，参数敏感性极低。

**原因分析**：`demand_multiplier=2.0` 网络压力过低，拥堵效应不明显，削弱博弈的空间差异化意义。

**敏感性测试** (`test_bf_demand_multiplier.py`)：

| Multiplier | 迭代次数 | GM | P95 | Completed |
|------------|---------|-----|-----|-----------|
| 2.0 | 4 | 1.60% | 10.34% | 100% |
| 2.1 | 5 | 1.76% | 11.11% | 100% |
| 2.2 | 5 | 1.96% | 12.50% | 100% |
| 2.3 | 7 | 2.70% | 16.13% | 100% |
| 2.4 | 9 | 2.49% | 14.29% | 100% |
| **2.5** | **25** | **2.75%** | **16.28%** | **100%** |

**结论**：在当前 γ=10, α=0.05 配置下，demand_multiplier=2.5 表现良好：
- 100% completed_ratio
- 25 轮迭代（有博弈意义）
- GM=2.75%, P95=16.28%

**已更新**：`berlin_friedrichshain_settings.json` → `demand_multiplier: 2.5`

### 3.4 更新 task_network_simplification.md

记录本次实验结果到文档 5.2.10.6 节。

## 4. 待解决问题：博弈环境中的热启动崩溃 (2025-12-20)

### 4.1 问题现象

在实际博弈训练中观察到 **Sioux Falls 网络完成率经常只有 60%**，与参数扫描验证时的 100% 完成率不符。

**典型日志**：
```
UE-DTA 第2轮 | 完成率:65.5% | Gap: GM=0.6% P90=1.6% P95=3.5% | 切换:119
UE-DTA 第2轮 | 完成率:60.1% | Gap: GM=0.6% P90=1.4% P95=2.9% | 切换:110
UE-DTA 第1轮 | 完成率:59.5% | Gap: GM=0.6% P90=1.5% P95=3.0% | 切换:101
```

**关键观察**：
- 问题发生在 **step 内部**的 UE-DTA 迭代中
- 完成率在某个 step **突然下跌**后保持稳定（不恢复）
- GM 仍然很低（0.6%），形成**虚假收敛**

### 4.2 问题机制分析

#### 4.2.1 验证实验 vs 博弈环境的差异

| 场景 | 价格 | 路径分配演化 | 完成率 |
|-----|------|-------------|--------|
| `verify_ue_parameters.py` | **固定** | 逐渐稳定 | 100% |
| 博弈环境 | **每 step 变化** | 需要不断适应 | 60% (崩溃后) |

#### 4.2.2 热启动崩溃机制

```
step 1: 完成率 100% ✓，路径分配 R1
                ↓ 热启动
step 2: 完成率 100% ✓，路径分配 R2
                ↓ 热启动
step N: 价格大幅变化 → 大量车辆切换 → 某路径过载 → 完成率 60% ✗
                ↓ 热启动（继承了"坏"的路径分配）
step N+1: 基于"坏"路径开始 → 完成率 60% ✗
                ↓ 热启动
step N+2: 完成率 60% ✗ ... 无法恢复
```

**核心问题**：热启动在完成率正常时加速收敛，但一旦"崩溃"就无法自我修复。

#### 4.2.3 虚假收敛

```
完成率: 60%  ←  40% 车辆未完成，不参与统计
GM: 0.6%    ←  只基于完成的 60% 车辆计算，看起来很好
```

当前 UE-DTA 收敛判断只检查 GM，不检查完成率，导致虚假收敛。

### 4.3 解决方案

#### 4.3.1 方案 1：收敛判断加入完成率约束（必须做）

**当前逻辑**：
```python
if GM < threshold:  # 只看 GM
    converged = True
```

**修改为**：
```python
if GM < threshold and completed_ratio >= 0.95:
    converged = True
```

**状态**：✅ 已实现 (2025-12-22)

**实现内容**：
1. 在三个网络的 `settings.json` 中添加 `ue_min_completed_ratio: 0.95` 参数
2. 修改 `EVCSChargingGameEnv.py` 读取新参数
3. 修改收敛判断逻辑，同时检查 GM 和完成率

**验证测试** (`test_greedy_completion.py`)：

连续 20 个随机价格 Step 测试结果：

| 指标 | 修改前（预期） | 修改后（实际） |
|-----|--------------|--------------|
| 虚假收敛 | 可能在2-3轮误判 | **0次** ✓ |
| 最终完成率 | 60%（错误收敛） | **100%** ✓ |
| 平均迭代次数 | 2-3轮（错误） | **30.9轮**（正确） |
| 收敛率 | 100%（虚假） | **100%**（真实） |

**结论**：完成率约束成功防止虚假收敛，代价是迭代次数增加（从2-3轮到平均31轮）。

#### 4.3.2 方案 2：热启动策略改进（待选择）

| 方案 | 描述 | 优点 | 缺点 |
|-----|------|------|------|
| **A. 条件重置** | 完成率 < 80% 时，放弃热启动，重新随机初始化 | 简单有效 | 可能反复重置 |
| **B. 局部修复** | 只对未完成车辆重新分配路径，已完成车辆保持 | 保留有效信息 | 实现稍复杂 |
| **C. 完全放弃热启动** | 每个 step 独立从贪心初始化开始 | 最稳定 | 计算时间增加 |
| **D. 扩大仿真时间** | 增加 simulation_time | 给车辆更多完成时间 | 治标不治本 |

**初步建议**：
- 方案 A 或 B 比较实际
- 方案 C 作为保底选项

**状态**：✅ 已选择方案3（见下文）

#### 4.3.3 方案 3：未完成车辆随机切换（已实现）✅

**决策依据**：
- 简单有效，无需复杂的成本估算
- 避免 `traveltime_actual` 在极度拥堵时的准确性问题
- 与 UXsim 官方的随机切换思路一致
- 符合"探索-利用"权衡：未完成=探索失败，需要更多探索

**实现逻辑**：

```python
# 未完成车辆：以固定概率随机切换
if veh.state != "end":
    if np.random.random() < self.ue_uncompleted_switch_prob:
        # 排除当前路径，随机选择其他路径
        other_routes = [r for r in available_routes if r != current_route]
        if other_routes:
            new_routes_specified[veh_id] = random.choice(other_routes)
```

**配置参数**：`ue_uncompleted_switch_prob: 0.3`（默认30%切换概率）

**优点**：
- ✅ 打破拥堵死锁
- ✅ 实现简单，性能开销小
- ✅ 30%概率避免羊群效应
- ✅ 可通过配置文件调整

**已实现**：
- [x] 配置参数添加到三个网络的 settings.json
- [x] 环境代码读取参数
- [x] 充电车辆和非充电车辆的随机切换逻辑
- [x] 切换次数统计（包含未完成车辆的切换）

**状态**：✅ 已完成 (2025-12-22)

### 4.4 待办事项

- [x] 实现收敛判断的完成率约束（方案 1）
- [x] 实现未完成车辆随机切换（方案 3）
- [x] 验证修复效果

## 5. 已完成工作计划

- [x] **Step 1**：检查 Sioux Falls 配置，分析 completed_ratio 异常原因
- [x] **Step 2**：修复评分函数，重新选择最佳参数
- [x] **Step 3**：更新三个网络的 `settings.json` 配置
- [x] **Step 4**：运行验证测试，确认改进效果
- [x] **Step 5**：更新 `task_network_simplification.md` 文档

## 6. 相关文件

- 参数扫描脚本：`sweep_ue_parameters.py`
- 扫描结果分析：`analyze_sweep_results.py`
- 扫描结果报告：`results/parameter_sweep/sweep_report.json`
- 可视化图表：`results/parameter_sweep/{network}_parameter_sweep.png`
- 环境实现：`src/env/EVCSChargingGameEnv.py`
- 网络配置：
  - `data/siouxfalls/siouxfalls_settings.json`
  - `data/berlin_friedrichshain/berlin_friedrichshain_settings.json`
  - `data/anaheim/anaheim_settings.json`

## 7. 协作记录

| 日期 | 进展 |
|-----|------|
| 2025-12-18 | 完成参数扫描实验，发现 Sioux Falls completed_ratio 异常 |
| 2025-12-18 | 分析问题根因：评分函数未考虑 completed_ratio，导致选出"虚假收敛"参数 |
| 2025-12-18 | 创建 `analyze_sweep_results.py`，添加 completed_ratio ≥ 95% 硬约束 |
| 2025-12-18 | 更新 Sioux Falls (γ=5, α=0.08) 和 Anaheim (γ=15, α=0.08) 参数 |
| 2025-12-18 | Berlin demand_multiplier 敏感性测试，从 2.0 提升到 2.5，迭代次数从 4 增加到 25 |
| 2025-12-20 | 发现博弈环境中 SF 网络完成率崩溃问题（60%），分析热启动机制导致的"崩溃后无法恢复" |
| 2025-12-20 | 提出解决方案：(1) 收敛判断加入完成率约束 (2) 热启动策略改进（条件重置/局部修复/完全放弃） |
| 2025-12-22 | 实现方案1：在 settings.json 添加 `ue_min_completed_ratio` 参数，修改收敛判断逻辑 |
| 2025-12-22 | 验证测试：连续20个随机价格step，收敛率100%，完成率100%，无虚假收敛 |
| 2025-12-22 | 确认方案1有效，迭代次数从2-3轮增加到平均31轮（正确代价） |
| 2025-12-22 | 分析未完成车辆问题：(1) 未完成车辆不切换会导致死锁 (2) `traveltime_actual` 在极度拥堵时失真 |
| 2025-12-22 | 实现方案3：未完成车辆随机切换功能，添加 `ue_uncompleted_switch_prob` 配置参数（默认0.3） |
| 2025-12-22 | 更新三个网络配置文件，实现充电/非充电车辆的随机切换逻辑，包含切换统计 |

