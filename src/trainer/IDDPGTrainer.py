...
class IDDPGTrainer:
    ...
    def __init__(self, task: ExperimentTask, monitor_config: Optional[MonitorConfig] = None):
        ...
        # IDDPG特有：计算局部状态维度
        # 局部状态 = 全局价格历史 + 自身流量 + 自身动作
        n_agents = len(self.env.agents)
        n_periods = action_dim
        local_state_dim = n_agents * n_periods + n_periods + n_periods

        # 3. 创建IDDPG算法
        self.iddpg = IndependentDDPG(
    ...
    def _run_episode(self, episode: int, observations: Dict) -> Tuple[bool, int]:
        ...
        with tqdm(total=self.config.max_steps_per_episode,
            ...
            for step in range(self.config.max_steps_per_episode):
                ...
                # 智能体决策（返回带噪音动作和纯策略输出）
                actions, pure_actions = self.iddpg.take_action(observations, add_noise=True)
                ...
                # 存储经验并学习
                self.iddpg.store_experience(observations, actions, rewards, next_observations, terminations)
                learn_metrics = self.iddpg.learn()
